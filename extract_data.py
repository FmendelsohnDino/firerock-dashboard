...
def extract(filepath):
    ...
    # Daily orders by account (optional ORDERS sheet, from the Daily Order Summary)
    if 'ORDERS' in wb.sheetnames:
        wo = wb['ORDERS']
        for row in range(2, wo.max_row + 1):
            cust = wo.cell(row, 3).value
            amt = wo.cell(row, 4).value
            if not cust or str(cust).strip().lower() == 'grand total':
                continue
            if not isinstance(amt, (int, float)):
                continue
            data['orders'].append({
                "rep":      str(wo.cell(row, 1).value or '').strip(),
                "so":       str(wo.cell(row, 2).value or '').strip(),
                "customer": str(cust).strip(),
                "amount":   amt,
                "note":     str(wo.cell(row, 5).value or '').strip()
            })
